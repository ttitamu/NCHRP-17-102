...
TITLE '6U+6D+8D';
DATA t; SET seg_2;
IF roadway_type IN ("6U","6D","8D");
IF I_full_twltl NE -1 THEN DELETE;
 RUN;
...
PROC NLMIXED  DATA=t_1  MAXIT=600  ;*CORR;
  PARMS  
 	     b0=-5  b_8d=0
 		b_tx=0 b_mn=0 /*just 2 pa - tx is base*/
		b_adt=0.5 b_ped=0 
		 b_lw=0 b_alc=0 b_lgt=0 b_park=0 b_sig2=0   
		 k_0=1  ;
  Wl_base = 12;
 
f_state=exp(b_tx*I_tx +b_mn*I_mn);
AF_lw=exp(b_lw*(lw-Wl_base));
AF_alc=EXP(b_alc*liquor_stores_0_1mi);
AF_lgt=EXP(b_lgt*I_lgt);
AF_park=EXP(b_park*I_park);
AF_sig2=EXP(b_sig2*I_sig2);

IF roadway_type IN ("6U") THEN DO;
p= b0 + b_adt*LOG(Avg_AADT) + b_ped*LOG(Avg_AADP) ;
    BASE=length_mi*duration*f_state *EXP(p);
    k=1/k_0;  END;

IF roadway_type IN ("6D") THEN DO;
p= b0 + b_adt*LOG(Avg_AADT) + b_ped*LOG(Avg_AADP) ;
    BASE=length_mi*duration*f_state *EXP(p);
    k=1/k_0;  END;

IF roadway_type IN ("8D") THEN DO;
p= b0 + b_8d+ b_adt*LOG(Avg_AADT) + b_ped*LOG(Avg_AADP) ;
    BASE=length_mi*duration*f_state *EXP(p);
    k=1/k_0;  END;

  _mu_= BASE*AF_lw*AF_alc*AF_lgt*AF_park*AF_sig2;
...
/*  OUTPUT OUT=RES PREDICTED=YHAT RESIDUAL=EHAT;*/;
